...
\begin{enumerate}[label=(\roman*)]
  ...
  Its organizational logic has been debated for millennia---Han Kangbo called it ``not the essence of the \textit{Yi},'' Zhu Xi deemed it ``unknowable,'' and Gao Heng argued it is meaningful---but no consensus exists \citep{rutt2002,shaughnessy2014}.
\end{enumerate}
...
\begin{remark}[Historical clarification]\label{rem:historical}
  ...
  Our analysis treats the two orderings as independently defined mathematical objects and studies the \emph{structural distance} between them, analogous to comparing two genome sequences without presupposing which is ancestral.
\end{remark}
...
\midrule
Number of cycles & 3 & Few (random expectation $\approx 4.4$) \\
...
\begin{enumerate}
  ...
  \item \textbf{Structural implication.} The King Wen sequence is not a random rearrangement of the binary natural order. It exhibits systematic restructuring: high coupling (81.25\% in one cycle), no preservation (zero fixed points), and a preference for contrast over gradual transition (mean Hamming distance 3.349 vs.\ baseline 3.0).
  ...
  \item \textbf{Historical nuance.} The binary natural order was formalized $\sim$2{,}100 years after the King Wen sequence. Our analysis measures the structural distance between two independently defined orderings---it does not presuppose that one was derived from the other.
\end{enumerate}
...
\bibitem[Moore]{moore}
Moore, S.~J.
Structural Analysis of the King Wen Sequence.
